...
def main():
    ...
    check("all five region groups discovered", all(x in names for x in ["🇺🇸 美国 [地区]","🇯🇵 日本 [地区]","🇸🇬 新加坡 [地区]","🇭🇰 香港 [地区]","🇹🇼 台湾 [地区]"]))
    check("three system strategies generated", all(x in names for x in ["♻️ 自动选择 [系统]","🛡️ 故障转移 [系统]","⚖️ 负载均衡 [系统]"]))
    check("Hrules fallback rule present", out["rules"][-1] == "MATCH,🚀 漏网之鱼 [自选]")
    manual_source = dict(source)
    manual_source["proxies"] = source["proxies"] + [{"name":"Premium-01","type":"http","server":"127.0.0.1","port":19006}]
    manual = real_builder.build(manual_source, template, ROOT / "rules", ROOT / "policies/v0.1.yaml", False, {"us":["Premium-01"]})
    by_name = {g["name"]: g for g in manual["proxy-groups"]}
    check("manual region node included", "Premium-01" in by_name["🇺🇸 美国 [地区]"]["proxies"])
